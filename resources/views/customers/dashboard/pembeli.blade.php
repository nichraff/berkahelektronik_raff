    ...
    <div id="bannerCarousel" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-indicators">
            @foreach($banners as $index => $banner)
                <button type="button" data-bs-target="#bannerCarousel" data-bs-slide-to="{{ $index }}" class="@if($index==0) active @endif"></button>
            @endforeach
        </div>
            ...
            @forelse($products as $product)
                @php
                    ...
                    if($imgUrl && str_contains($imgUrl, 'drive.google.com')) {
                        ...
                        } elseif(preg_match('/id=([a-zA-Z0-9_-]+)/', $imgUrl, $matches)) {
                            $imgUrl = 'https://drive.google.com/uc?id='.$matches[1];
                        }
                    }
